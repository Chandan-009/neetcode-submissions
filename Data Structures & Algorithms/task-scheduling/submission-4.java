class Solution {
        public int leastInterval(char[] tasks, int n) {
            ...
            while(pq.size()>0 || q.size()>0) {
                char c='N';
                // check queue
                if(q.size()>0 && (q.peek().next_index==current_index)){
                    pq.add(q.poll());
                }
                // check heap
                if(pq.size()>0){
                    ...
                    cur_task.next_index=current_index+n+1;
                    // add it to the queue
                    System.out.println(cur_task);
        ...
        class Task implements Comparable<Task> {
            ...
            int next_index=-1;
            int frequnecy;
            Task(char name,int next_index, int frequnecy){
                this.name=name;
            ...
            public String toString(){
                return this.name+" "+this.next_index+" "+this.frequnecy;
            }
